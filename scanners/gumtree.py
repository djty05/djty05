...
class GumtreeScanner(BaseScanner):
            ...
            try:
                ...
                if isinstance(data, dict) and data.get("@type") == "ItemList":
                    for item in data.get("itemListElement", []):
                        ...
                        if obj.get("image"):
                            img_val = obj["image"]
                            img = img_val[0] if isinstance(img_val, list) and img_val else str(img_val)
                        results.append(Listing(
                            title=obj.get("name", "No title"),
                            price=price, url=item_url,
    ...
    def _parse_html(self, html: str, term: str) -> list[Listing]:
        ...
        selectors = [
            "a.user-ad-row", "a.listing-link", "div.user-ad-row",
            "a[data-testid='listing-card']", "div[data-testid='listing-card']",
            "article a", "div.srp-item a",
        ]
        ...
        for card in cards:
            try:
                title_el = card.select_one(
                    "span.user-ad-row__title, h3.listing-title, p.tempo-title, "
                    "div.user-ad-row__title, h2, h3, [data-testid='listing-title']"
                )
                price_el = card.select_one(
                    "span.user-ad-price, div.listing-price, p.tempo-price, "
                    "div.user-ad-row__price, [data-testid='listing-price']"
                )
    ...
    def _generate_direct_links(self) -> list[Listing]:
        ...
        for term in self.search_terms[:4]:
            ...
            links.append(Listing(
                title=f"Gumtree: {term}", price="Open Gumtree", url=url,
                location="Australia", marketplace=self.name,
